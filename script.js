...
function renderHistoryData(data) {
  ...
  if (!data || data.length === 0) {
    tbody.innerHTML =
      '<tr class="empty-msg"><td colspan="7" class="p-16 text-center text-slate-400 border-b-0"><div class="flex flex-col items-center justify-center"><i class="fa-regular fa-folder-open text-4xl mb-3 text-slate-300"></i><p>Chưa có giao dịch nào được ghi nhận.</p></div></td></tr>';
    return;
  }
...
function formatCurrencyInput(e) {
  ...
  // Kích hoạt tính toán lợi nhuận ngay khi gõ giá thu
  autoCalculateProfit();
}
...
function updateServiceLabels() {
  ...
  blocks.forEach((block, index) => {
    ...
    if (label)
      label.innerHTML = `Loại dịch vụ ${index + 1} <span class="text-red-500">*</span>`;
    if (index === 0) {
      ...
      if (!deleteBtn) {
        ...
        btn.type = "button";
        btn.className =
          "delete-btn absolute top-3 right-3 text-slate-400 hover:text-red-500 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-red-50 transition-colors";
...
function renderFields(blockId) {
  ...
  if (serviceName && CONFIG_SERVICES[serviceName]) {
    ...
    CONFIG_SERVICES[serviceName].forEach((field) => {
      ...
      const currencySpan = field.isCurrency
        ...
        : "";
      // Gắn sự kiện autoCalculateProfit vào tất cả các ô input khi người dùng gõ
      const onInputEvt = field.isCurrency
        ? `oninput="formatCurrencyInput(event); autoCalculateProfit();"`
        : `oninput="autoCalculateProfit();"`;
  ...
  checkGlobalRules();
  checkGlobalRules();
  autoCalculateProfit(); // Thêm dòng này để reset lợi nhuận khi đổi loại dịch vụ
}
...
function checkGlobalRules() {
  ...
  const lastService =
    selects.length > 0 ? selects[selects.length - 1].value : "";
  const giaThuInput = document.getElementById("gia-thu");

  if (
    firstService === "BÁN SIM" &&
    lastService === "ĐĂNG KÝ GÓI CƯỚC DI ĐỘNG"
  ) {
...
async function sendDataToSheets() {
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    for (let j = 0; j < inputs.length; j++) {
      ...
      if (isRequired && inputs[j].value.trim() === "")
        return alert(
          `Vui lòng điền đủ các thông tin bắt buộc cho dịch vụ "${sName}"!`,
        );
    }
  }
  ...
  submitBtn.innerHTML =
    '<i class="fa-solid fa-spinner fa-spin mr-2"></i> ĐANG GHI NHẬN...';
  ...
  blocks.forEach((block, index) => {
    ...
    let packageData = {
      ...
      gia_thu:
        index === 0
          ? getRawNumber(document.getElementById("gia-thu").value)
          : "",
      method: index === 0 ? document.getElementById("method").value : "",
      profit:
        index === 0
          ? getRawNumber(document.getElementById("profit").value)
          : "",
...
function addOrderToTable(orderArray, orderId, isFromServer = false) {
  ...
  orderArray.forEach((data, index) => {
    ...
    const methodStyle =
      data.method === "CHUYỂN KHOẢN"
        ? "bg-emerald-100 text-emerald-700"
        : data.method === "TIỀN MẶT"
...
function autoCalculateProfit() {
  // 1. Lấy giá trị Giá thu hiện tại
  const giaThuInput = document.getElementById("gia-thu").value;
